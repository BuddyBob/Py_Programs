...
def full(YourPath):
    #Open json file
    with open(str(YourPath)+"Covid-19/DeathStats/Deaths/similar.json", "r") as read_file:
        data = json.load(read_file)
        #Store Deaths and Countries
        keys = list(data.keys())
        values = list(data.values())
    #Open DeathStats file
    check = open(str(YourPath)+"Covid-19/DeathStats/Deaths/DeathStats.txt")
    #Create file to write all rows of data
    lastFile = open(str(YourPath)+"Covid-19/DeathStats/Deaths/Final.txt",'w')
    L = []

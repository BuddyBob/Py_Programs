...
def full(YourPath):
    #Open json file
    with open(str(YourPath)+"Hackathon/RecoveredCases/Recovered/similar.json", "r") as read_file:
        data = json.load(read_file)
        #Store Deaths and Countries
        keys = list(data.keys())
        values = list(data.values())
    #Open Recovered file
    check = open(str(YourPath)+"Hackathon/RecoveredCases/Recovered/Recovered.txt")
    #Create file to write all rows of data
    lastFile = open(str(YourPath)+"Hackathon/RecoveredCases/Recovered/Final.txt",'w')
    L = []
